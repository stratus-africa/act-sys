...
function Overview() {
  ...
  const save = async () => {
    ...
    toast.success("Saved");
    window.dispatchEvent(new CustomEvent("patient:refresh", { detail: { patientId } }));
  };
  ...
  const F = ({ k, label, type = "text" }: { k: string; label: string; type?: string }) => (
    <div>
      ...
      <input type={type} disabled={!canEdit} value={p[k] ?? ""} onChange={(e) => setP({ ...p, [k]: e.target.value })} className="w-full px-3 py-2 border border-border bg-background text-sm disabled:opacity-60" />
    </div>
  );
      ...
      <section className="border border-border bg-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div>
            ...
            <select disabled={!canEdit} value={p.general_condition ?? ""} onChange={(e) => setP({ ...p, general_condition: e.target.value || null })} className="w-full px-3 py-2 border border-border bg-background text-sm disabled:opacity-60">
              <option value="">—</option>
          ...
          <label className="flex items-center gap-2 text-sm pt-6"><input type="checkbox" disabled={!canEdit} checked={p.dnr_status} onChange={(e) => setP({ ...p, dnr_status: e.target.checked })} className="accent-primary" />DNR status</label>
        </div>
      </section>
      {canEdit && (
